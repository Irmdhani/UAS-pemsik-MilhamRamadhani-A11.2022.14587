...
import { UserPlus, Loader2, GraduationCap, User, MapPin, Contact, CalendarClock, Save, X } from 'lucide-react';
...
const Content = () => {
    ...
    const handleAddStudent = async (e) => {
        ...
        if (
            ...
            !newStudent.umur.trim() ||
            !newStudent.keterangan.trim() 
        ) {
            Swal.fire({
                icon: 'error',
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-1 p-4 bg-gradient-to-r from-amber-100 to-amber-200 rounded-lg shadow-sm">
                <div className="flex items-center gap-2">
                    <FaUser className="text-red-700" size={24} />
                    <h2 className="text-xl font-semibold text-gray-800">Korban Bencana Tanah Longsor</h2>
                </div>
                <Button
                    label={
                        <div className="flex items-center gap-2">
            ...
            <main className="flex-grow p-4 bg-gradient-to-r from-slate-50 to-slate-150 rounded-lg shadow-sm">
                {status === 'loading' ? (
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center backdrop-blur-sm">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-1/3 max-w-md">
                        <div className="flex items-center gap-2 mb-4">
                            <GraduationCap className="text-amber-950" size={24} />
                        ...
                        <form onSubmit={handleAddStudent} className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <input
                                    id="progdi_id"
                                    type="text"
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-900 focus:border-orange-900 transition-all duration-200"
                                    value={newStudent.id}
